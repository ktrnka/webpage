...
- **Suhara et al., 2020.** [OpinionDigest: A Simple Framework for Opinion Summarization](https://aclanthology.org/2020.acl-main.513/). *ACL.* ✅ - Aspect-based abstractive summarization over many reviews (e.g. Yelp): extract opinion phrases, cluster, synthesize. This was an inspiration for how I did review summarization, though it works at a finer aspect granularity. Where it didn't transfer was multi-source - its best-review / worst-review selection would work on something like Glassdoor, but not on Reddit and not across CD's mix of sources.
- **Tan et al., 2020.** [Summarizing Text on Any Aspects](https://aclanthology.org/2020.emnlp-main.510/). *EMNLP.* ✅ - Aspect-conditioned summarization that deliberately covers a *diverse* set of aspects so the summary is comprehensive. The closest inspiration for my product / employee / financial aspects. Using ConceptNet is a neat move, but it makes the whole approach dependent on that graph's coverage.
...
- **Khosravani and Trabelsi, 2023.** [Recent Trends in Unsupervised Summarization](https://arxiv.org/abs/2305.11231). *arXiv:2305.11231.* - A great survey of the space; the kind of read that pushes you toward an extract-then-abstract *hybrid* rather than purely extractive or purely abstractive. Their sections on that split make the case that extract-then-abstract does especially well in long or multi-document settings, which is exactly the situation CD was in.
- **Zeng et al., 2023.** [Meta-review Generation with Checklist-guided Iterative Introspection](https://arxiv.org/abs/2305.14647v1). *arXiv:2305.14647v1.* - Checklist-guided iterative self-critique to synthesize a meta-review from many paper reviews. Loosely adjacent to component-level quality checks (and a reminder that meta-reviews come with training data, which CD didn't have). The evaluation was the valuable part - especially *abstractiveness*, the percentage of n-grams in the summary that don't appear anywhere in the sources. The paper has since been revised and retitled around its dataset contribution; that later version appeared at the [IJCAI 2024 AI4Research Workshop](https://arxiv.org/abs/2305.14647).
- **Chhikara et al., 2024.** [LaMSUM: Creating Extractive Summaries of User Generated Content using LLMs](https://arxiv.org/abs/2406.15809v2). *arXiv:2406.15809v2.* - Getting genuinely *extractive* summaries out of an LLM, which doesn't come naturally since they paraphrase by default. It summarizes in chunks and uses voting algorithms to decide which of the original texts survive. Neat work, and close to CD's problem of picking a representative subset out of a big pile of user-generated text. The paper has since been substantially revised and retitled around a specific application; that later version is [peer-reviewed at ICWSM 2026](https://arxiv.org/abs/2406.15809).
...
- **Zhang et al., 2024.** [Benchmarking LLMs for News Summarization](https://aclanthology.org/2024.tacl-1.3/). *TACL (arXiv:2301.13848).* ✅ - Two takeaways: LLMs are strong at this, and the *reference* summaries in standard benchmarks are low quality, so reference-based eval can be risky if you don't verify them. I like that they didn't stop at automatic metrics and ran a human preference study.
...
- **Cohen-Wang et al., 2024.** [ContextCite: Attributing Model Generation to Context](https://proceedings.neurips.cc/paper_files/paper/2024/hash/adbea136219b64db96a9941e4249a857-Abstract-Conference.html). *NeurIPS.* ✅ - Attributes each part of an output to specific context via delete-and-rerun sensitivity - essentially LIME extended to context-based generation. Compelling for verifying attribution, but the delete-and-rerun approach needs a lot of extra model calls, which against a hosted API drives up both the bill and the latency.
